 ...
 .then ( data => {
  ...
  const category = todosEvents.map (events => events.category)
...
//  //imprimir
  ...
  function template ( lista ) {
    return lista.reduce( ( acc, act ) => {
        ...
        <img src="${act.image}" class="card-img-top object-fit-fill p-2" alt="books">
        <div class="card-body">
  ...
  cartas.innerHTML = template (todosEvents)


  inputSearch.addEventListener ( 'input', () => {
    const busqueda = filterTittle ( todosEvents, inputSearch.value)
